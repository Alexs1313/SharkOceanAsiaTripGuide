...
const Oceatrippguiddefactts = () => {
  ...
  const goCategory = (category: OceatrippguidFactsCategory) => {
    (stackNav ?? navigation).navigate('Oceatrippguiddefacttsdet', {
      category,
      fact: pickRandom(oceatrippguidFactsByCategory[category]),
    });
  };
    ...
    <Oceatrippguiddelayoutt>
      <View style={styles.oceatrippguidScreen}>
        ...
        <View style={styles.oceatrippguidCategories}>
          {oceatrippguidFactsCategories.map(c => (
            <Pressable key={c.key} onPress={() => goCategory(c.title)}>
              <LinearGradient
                colors={['#009BFF', '#005D99']}
        ...
        <Pressable onPress={goRandom} style={styles.oceatrippguidRandomWrap}>
          <LinearGradient
